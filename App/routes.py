...
@App.route('/extend_location')
@login_required
def extend_location():
    if current_user.is_authenticated and User.email == App.config.ADMINS:
        form = AdminLocationForm()
        if form.validate_on_submit():
            longlatradius = User(longitude=form.LocationRadius.data)
            db.session.add(longlatradius)
            db.session.commit()
            flash(_('Longitude and Lattitude updated'))
            return redirect(url_for('login'))
        return render_template("admin_index.html",title=l("Extend Location"), form=form)
    else:
        return redirect(url_for('index'))
...
def follow(username):
    ...
    if user == current_user:
        flash(_('Unable to follow self sorry'))
...
def unfollow(username):
    ...
    if user is None:
        ...
        return redirect(url_for('index'))
    if user == current_user:
        flash(_('Unable to unfollow %(username)s, must be really important.', username=username))
        return redirect(url_for('user', username=username))
